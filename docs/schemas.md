...
- **Security properties** — [SECURITY.md](../SECURITY.md)
 ...
 # XSPM Findings Model
  ...
  The data contract every XSPM assessor reads from and writes to — and the spec for `src/xspm/findings/schema.py`.

  **Version 0.1 (draft).** Design: cross-domain core + application extension, locked 2026-05-19. One validation check remains before v1.0 — see §9.
  ...
  **It carries through every build stage unchanged.** The wrapper (Aikido-backed) and the Kainos-owned platform that follows it produce the *identical* schema — only the parser input differs (Aikido JSON vs normalised SARIF). A later
  stage persists it in a database; the XSPM-module stage ingests it into the cross-domain dashboard. There is no second findings model later. This is it.

  **Why this spec exists.** Earlier internal notes described a findings model in two incompatible shapes — a cross-domain schema in one, a flat application-security-only model in another. Both claimed the schema would reach the XSPM
  platform with no migration work; only a domain-shaped schema makes that true. This spec reconciles them into one model; §8 records each change and its reason.

  ---

  ## 2. Design shape — core + extension

  Every security finding, whatever the domain, shares a spine: an identity, a severity, a status, a thing it is attached to, controls it implicates, remediation. What differs is the domain detail. So the model is a **stable core**
  every domain fills, plus a **typed payload** for the domain-specific fields. The wrapper produces only `application` findings, so only the `ApplicationDetail` payload exists today.

  ```
  FindingsDocument
  ├── schema_version
  ├── scan: ScanMeta              (scan_id, scanned_at, mode, target, engine, versions)
  ...
  Adding a domain later (DSPA, cloud, AI) is purely additive: define a new payload class, add it to the `domain` enum's payload map, touch nothing else. The report generator and overlay only ever read the core, so they never break.
  ...
  The full `Domain` and `Framework` enums are defined now even though the wrapper uses only `application` / `asvs` + `caf`. Enum members are string labels — defining them costs nothing and reserves the namespace. The premature-design
  risk lives in the *payload classes*, not the labels, so no payload class beyond `ApplicationDetail` is written here.
  ...
  class ControlRef(_Base):
      ...
      framework: Framework
      control_id: str                 # "5.3.4" (ASVS 5.0), "B3.a" (CAF v4.0)
  ...
  class Finding(_Base):
      ...
      # --- framework mapping (filled by the normaliser via the CWE->ASVS/CAF tables) ---
  ...
  The validator enforces the one invariant that matters: the payload set must match `domain`. In the wrapper every finding is `domain="application"` with an `ApplicationDetail`. When `DataDetail` is added later it joins the `payloads`
   map and the validator covers it for free.
  ...
  class ScanMeta(_Base):
      ...
      framework_versions: dict[str, str] = Field(default_factory=dict)  # {"asvs": "5.0", "caf": "v4.0"}
  ...
  `schema_version` is mandatory and bumped on any breaking change — schema versioning is in place from day one. Framework versions (ASVS 5.0, CAF v4.0) live here, on the scan, not on each finding — they are a scan-level fact, not
  repeated 200 times.
  ...
  The `id` is **not** a per-run UUID. It is a deterministic hash of a canonical fingerprint, so the *same vulnerability in the same place* gets the *same id on every scan*. That is what lets a later stage add trend-tracking with no
  schema change — "is this finding still open three scans later?" is just an id match.
  ...
  **Fingerprint recipe — `|`-joined, lowercased, pillar-specific:**
  ...
  Deduplication is not a separate algorithm. Two raw findings with the same fingerprint produce the same `id` — they are the same finding. The normaliser merges them: union `source_tools`, union `tool_rule_ids`, keep the highest
  severity.
  ...
  | `status` | `open`; set to `false_positive` if the consultant marks it during review |
  ...
  | `control_refs` | Filled via the CWE→ASVS 5.0 + CWE→CAF v4.0 tables — the IP layer |
  ...
  **Not stored anywhere:** posture score, ASVS coverage heatmap, CAF gap map. All three are *projections* the report generator computes from the findings. Storing a derived value invites drift — the report generator recomputes them
  every run.
  ...
  One SAST finding, serialised. Control identifiers are **illustrative** — exact ASVS 5.0 / CAF v4.0 ids come from the mapping tables (§10), not from this spec.
  ...
  ```json
  {
    ...
    "scan": {
      ...
      "framework_versions": {"asvs": "5.0", "caf": "v4.0"}
    },
    "findings": [
      {
        "id": "9b1c4e7a2f8d0a16",
        ...
        "control_refs": [
          {"framework": "asvs", "control_id": "5.3.4", "control_name": "Injection prevention", "level": "L1", "note": null},
          {"framework": "caf", "control_id": "B3.a", "control_name": "Understanding data", "level": null, "note": "Unmitigated injection weakens the data-protection claim in a CAF B3 submission."}
        ],
  ...
  | 3 | Status | open / false positive / accepted risk / resolved | open / resolved / false_positive / accepted_risk / **not_assessed** | Adds `not_assessed` for cross-domain use; snake_case for enum hygiene |
  ...
  | 10 | Consultant review | Status only — no reviewer, time, or rationale | Adds a `review` object: `reviewed_by` / `reviewed_at` / `note` | Status records the decision; the audit trail needs who reviewed it and why |

  ---

  ## 9. Status — one check before this is final

  This is draft v0.1. The schema is designed against SARIF and the public CWE / ASVS / CAF standards, so it can be implemented now without waiting on anything external.

  One check remains before it is declared final (v1.0): **validate it against a real Aikido findings export** — confirm an Aikido finding actually carries a usable CWE, a CVSS score, and a rule id. That is a check on the parser's 
  input assumptions, not on the schema's shape; it does not block writing `schema.py`.
  ...
  - **The CWE→ASVS 5.0 and CWE→CAF v4.0 mapping tables** — the IP layer. This schema provides the *slots* (`control_refs`); the tables are the next build artifact after the schema. They are reference data, not part of the schema.
  ...
  **Acceptance criteria:**
  ...
  - Hypothesis property test confirms the round-trip across generated findings
